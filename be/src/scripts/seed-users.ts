...
async function seedUsers() {
  ...
  try {
    ...
    if (existingUsers > 0) {
      Logger.log(`⚠️  기존 사용자 ${existingUsers}명이 있습니다.`);
    ...
    Logger.log(`총 ${mockUsers.length}명의 사용자를 삽입합니다...`);
    ...
    Logger.log(`✅ ${mockUsers.length}명의 사용자가 성공적으로 삽입되었습니다.`);
  } catch (error) {
    Logger.error('❌ 사용자 시드 중 오류 발생:', error);
    throw error;
  } finally {
